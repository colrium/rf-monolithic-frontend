...
class Utilities {
	...
	static isOfType(input, expect) {
		return (
			(expect.constructor === String &&
				((["array", "arr"].includes(expect.toLowerCase()) &&
					input !== undefined &&
					input !== null &&
					input.constructor === Array) ||
					(["boolean", "bool"].includes(expect.toLowerCase()) &&
						input !== undefined &&
						input !== null &&
						input.constructor === Boolean) ||
					(["function", "method", "fn", "callable"].includes(
						expect.toLowerCase()
					) &&
						input !== undefined &&
						input !== null &&
					...
					(["number", "float", "int", "long"].includes(
						expect.toLowerCase()
					) &&
						input !== undefined &&
						input !== null &&
						...
						input !== NaN) ||
					(["string", "str"].includes(expect.toLowerCase()) &&
						input !== undefined &&
						input !== null &&
						input.constructor === String) ||
					(["regexp", "regex"].includes(expect.toLowerCase()) &&
						input !== undefined &&
						input !== null &&
						input.constructor === RegExp) ||
					(["error", "err"].includes(expect.toLowerCase()) &&
						input !== undefined &&
						input !== null &&
						input instanceof Error) ||
					(["object", "obj"].includes(expect.toLowerCase()) &&
						input !== undefined &&
						input !== null &&
						typeof input === "object" &&
						input.constructor !== Number &&
						input.constructor !== NaN &&
						input.constructor !== String &&
						input.constructor !== RegExp &&
						input.constructor !== Boolean &&
...
class ServiceData {
	resolveReferenceColumnDisplay(
		...
		if (Utilities.isOfType(resolves, "object")) {
			...
			if (Utilities.isOfType(entry[column], "object")) {
					...
					for (var j = 0; j < primary_resolves.length; j++) {
						if (Utilities.isOfType(primary_resolves[j], "object")) {
							for (let [
								primary_key,
								primary_value,
							] of Object.entries(primary_resolves[j])) {
								if (
									Utilities.isOfType(
							...
							Utilities.isOfType(primary_resolves[j], "string") &&
							entry[column][primary_resolves[j]]
						) {
							primary_resolve_value +=
								" " + entry[column][primary_resolves[j]];
						}
				...
				if (
					Utilities.isOfType(secondary_resolves, "array") &&
					secondary_resolves.length > 0
				) {
					...
					for (var k = 0; k < secondary_resolves.length; k++) {
						if (secondary_resolves[k] in entry[column]) {
							secondary_resolve_value +=
								" " + entry[column][secondary_resolves[k]];
						}
					}
					column_resolve +=
					...
					if (
						...
						for (var j = 0; j < primary_resolves.length; j++) {
							if (
								...
								entry[column][x][primary_resolves[j]]
							) {
								primary_resolve_value +=
									" " + entry[column][x][primary_resolves[j]];
							}
					...
					if (
						...
						for (var k = 0; k < secondary_resolves.length; k++) {
							if (secondary_resolves[k] in entry[column][x]) {
								secondary_resolve_value +=
									" " +
									entry[column][x][secondary_resolves[k]];
							}
						}
						item_resolve +=
							secondary_resolve_value.trim().length > 0
	...
	resolveReferenceColumnsDisplays(
		entries,
		columns,
		user,
		secondary_prefix = "(",
		...
		if (Utilities.isOfType(columns, "object")) {
			for (let [name, properties] of Object.entries(columns)) {
				if (properties.reference) {
					if (
						Utilities.isOfType(properties.reference, "object") &&
						"resolves" in properties.reference
					) {
				...
				for (let [field, value] of Object.entries(entries[i])) {
					if (field in columns_resolves) {
						if (entries[i][field]) {
							resolved_entry[
								field
							] = this.resolveReferenceColumnDisplay(
								field,
								entries[i],
								columns_resolves[field],
								user,
								secondary_prefix,
								secondary_suffix
							);
						...
						if (JSON.isJSON(entries[i][field])) {
							resolved_entry[field] = JSON.prettyStringify(
								entries[i][field]
							);
						} else {
							resolved_entry[field] = entries[i][field];
