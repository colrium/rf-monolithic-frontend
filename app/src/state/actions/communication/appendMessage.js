...
export default function appendMessage(message, new_incoming = false) {
	return async (dispatch, getState) => {
		...
		if (auth.isAuthenticated) {
			...
			let fetchMessageFromServer = !is_message_obj
			...
			if (JSON.isJSON(message)) {
				const { conversation, sender, uuid } = message
				...
				const index_of_conversation = getIndexOfConversation(conversations, conversation_uuid || conversation_id)
				const is_current_active_conversation = !String.isEmpty(active_conversation) && active_conversation_index !== -1 && active_conversation_index === index_of_conversation

				let conversation_obj = conversations[index_of_conversation]

				let newConversations = [...conversations]
				if (index_of_conversation === -1 || JSON.isEmpty(conversation_obj)) {
				...
				try {
					if (new_incoming) {
						if (active_conversation_index !== index_of_conversation || (active_conversation_index === index_of_conversation && currentScreenName !== "MessagesScreen")) {
							if ((String.isEmpty(senderName) || senderName === sender_id) && !JSON.isEmpty(conversation_obj) && Array.isArray(conversation_obj?.participants)) {
								conversation_obj.participants.map(participant => {
						...
						if (message.state === "sent") {
							if (active_conversation_index === index_of_conversation) {
								if (currentScreenName == "MessagesScreen") {
									...
									EventRegister.emit("mark-message-as-received", {
										message: message._id,
										user: auth.user?._id || auth.user,
									})
								}
					...
					if (!JSON.isEmpty(conversation_obj)) {
						let messageConversationMessagesIndex = getIndexOfMessage(conversation_obj.messages, message)
						let is_last_message = conversation_obj.state?.last_message?.uuid === message.uuid || conversation_obj.state?.last_message?._id === message._id
						let last_message = is_last_message ? JSON.merge(conversation_obj.state?.last_message, message) : message
						...
						if (messageConversationMessagesIndex === -1) {
							conversation_obj.messages = concatMessages(conversation_obj.messages, message)
							messageConversationMessagesIndex = getIndexOfMessage(conversation_obj.messages, message)
							if (new_incoming && !is_current_active_conversation) {
								conversation_obj.state.incoming_unread = Number.parseNumber(conversation_obj.state.incoming_unread || 0) + 1
							}
							conversation_obj.state.total = Number.parseNumber(conversation_obj.state.total || 0) + 1
							conversation_obj.state.last_message = message
						} else {
							is_last_message = conversation_obj.state?.last_message?.uuid === message.uuid || conversation_obj.state?.last_message?._id === message._id
							last_message = is_last_message ? JSON.merge(conversation_obj.state?.last_message || {}, message) : message
							conversation_obj.messages[messageConversationMessagesIndex] = JSON.merge(conversation_obj.messages[messageConversationMessagesIndex], message)
						}
						if (index_of_conversation > 0) {
							newConversations.splice(index_of_conversation, 1)
							if (!Array.isArray(newConversations)) {
								newConversations = []
							}
							newConversations.unshift(conversation_obj)
						}
