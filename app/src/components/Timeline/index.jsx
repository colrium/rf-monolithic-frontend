	...
	return (
		(activity.left <= _activity.left &&
			activity.left + activity.width >=
				_activity.left + _activity.width) ||
		(activity.left >= _activity.left &&
			activity.left <= _activity.left + _activity.width)
	);
};
	...
	calculateData(props) {
		...
		const positionedActivities = activities
			...
			.reduce((previous, activity) => {
				...
				activity.width =
					Math.max(1, stopIndex - startIndex) * props.flexBasis;

				const lineIndexes = previous
					.filter(
						_activity =>
							overlap(activity, _activity) ||
							tooClose(activity, _activity)
					)
	...
	onMouseMove = e => {
		...
		const max =
			typeof e.currentTarget.querySelectorAll === "function"
				? Array.from(
						e.currentTarget.querySelectorAll(".activity")
		...
		const baseX =
			this.state.baseX.indexOf("%") !== -1
				? Math.round(
						(parseInt(this.state.baseX, 10) / 100) * rulerWidth
				  )
