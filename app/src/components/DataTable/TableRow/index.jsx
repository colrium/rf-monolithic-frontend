...
export const DataTableRow = React.forwardRef((props, ref) => {
	const { data, headers, selected, collapsible, collapseHeaders, collapseContent, openRows, selected, rowSpan, onClick, onContextMenu, index, identifier, disableCopyToClipboard, truncate, ...rest } = props;
    ...
    const getCellDom = useCallback((header, data) => {
    ...
    return (
									<Fragment key={data[identifier] || index}>
										<TableRow
											...
											sx={{
												opacity: isDisabled(data)
													? 0.3
													: 1,
												cursor: isSelectible(data)
													? "pointer"
													: "auto",
											}}>
											{headers.map(header => {
