...
class ApiData extends React.PureComponent {
	constructor(props) {
		...
		this.handleSocketsReconnected =
			this.handleSocketsReconnected.bind(this);
	...
	getLocalStorageData(scope = false) {
		let cacheState = String.isEmpty(scope)
			? { data: [], pagination: { pages: 0, page: 0, count: 0 } }
			: {};
		try {
			let apiCache = localStorage.getItem("api-cache")
				? JSON.parse(localStorage.getItem("api-cache"))
				: {};
			cacheState = String.isEmpty(scope)
				? apiCache
				: { ...cacheState, ...apiCache[scope] };
		} catch (error) {
	...
	getLocalStorageDataItemById(scope = false, id = "") {
		...
		try {
			...
			if (Array.isArray(cacheState?.data)) {
				indexIncacheState = cacheState.data.findIndex(
					entry => entry?.uuid === id || entry?._id === id
				);
	...
	persistToCache(
		scope,
		value = { data: [], pagination: { pages: 0, page: 0, count: 0 } },
		action = "append"
	) {
		...
		if (Array.isArray(cacheValue?.data)) {
			cachedIds = cacheValue.data.reduce(
				(currentCachedIds, cachedEntry) => {
				...
				(currentNewCachedData, entry) => {
					...
					if (cacheIndex !== -1) {
						if (action === "delete") {
							...
							currentNewCachedData[cacheIndex] = JSON.merge(
								cacheValue.data[cacheIndex],
								entry
							);
						}
					} else {
		...
		let localcache = this.setLocalStorageData(scope, {
			data: newCachedData,
						...
						Api.get(uri, { ...reqConfig })
							.then(res => {
								...
								if (persist && !JSON.isEmpty(data)) {
									let cachedData = this.persistToCache(
										scope,
										{ data: [data] }
									);
								}
							...
							.catch(error => {
								...
								reject({
									data: this.getLocalStorageDataItemById(
										scope,
										id
									),
									error,
	...
	postMany =
		...
		(data = [], options = {}) => {
			...
			return new Promise((resolve, reject) => {
				Promise.allSettled(
					data.map(entry => apiRequest(data, options))
				).then(res => {
					resolve(res);
				});
			});
	...
	initializeData() {
		...
		let initialData = {};
		initialData = Object.entries(definations).reduce(
			(currentValue, [name, defination]) => {
				currentValue[name] = {
					data: [],
