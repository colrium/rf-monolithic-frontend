...
class GoogleMapOverview extends React.Component {
	...
	constructor(props) {
		super(props);
		const { device, cache, context, contexts, definations, app } =
			this.props;
		...
		this.handleClientListItemClick =
			this.handleClientListItemClick.bind(this);
		this.handleOnToggleShowClients =
			this.handleOnToggleShowClients.bind(this);
		this.handleOnClientsPositionsLoaded =
			this.handleOnClientsPositionsLoaded.bind(this);
		this.handleOnClientPositionAvailable =
			this.handleOnClientPositionAvailable.bind(this);
		this.handleOnClientPositionUnavailable =
			this.handleOnClientPositionUnavailable.bind(this);
		this.handleOnClientPositionChanged =
			this.handleOnClientPositionChanged.bind(this);
		this.handleOnDrawerNavigationValueChange =
			this.handleOnDrawerNavigationValueChange.bind(this);
		this.handleContextEntryClick = this.handleContextEntryClick.bind(this);
		this.handleToggleContextRender =
			this.handleToggleContextRender.bind(this);
		...
		this.state = {
			...
			multicontexts: Array.isArray(contexts)
				? contexts.length > 1
				: false,
			...
			contexts_props: context
				? {
						[context]: {
							query: {
								page: 1,
								pagination: app.preferences.data.pagination,
								populate: 1,
							},
							element: "marker",
							records: [],
				...
				: {},
			context_entries: Array.isArray(
				cache.data[this.state.defination.name]
			)
				? cache.data[this.state.defination.name]
				: [],
		...
		for (let [name, defination] of Object.entries(definations)) {
			if (!defination.access.restricted(auth.user)) {
				if (
					...
					possible_services[name] = ApiService.getContextRequests(
						defination.endpoint
					);
					contexts_props[name] = {
						query: {
					...
					if (current_defination === null) {
						...
						current_service = ApiService.getContextRequests(
							defination.endpoint
						);
					}
				}
	...
	handleOnToggleShowClients(event) {
		this.setState(state => ({
			show_client_positions: !state.show_client_positions,
		}));
	}
	...
	handleOnClientPositionChanged(socketId, client) {
		this.setState(state => {
			return {
				clients_positions: state.drawerOpen
					? { ...state.clients_positions, [socketId]: client }
					: state.clients_positions,
			};
		});
	}
	...
	handleOnDrawerNavigationValueChange = (event, newValue) => {
		...
		this.setState(prevState => {
			let renderedContextIndex =
				prevState.rendered_contexts.indexOf(newValue);
			let rendered_contexts =
				renderedContextIndex !== -1
					? prevState.rendered_contexts
					: prevState.rendered_contexts.concat([newValue]);
			return {
				context: newValue,
				context_entry: null,
				context_entries: ["commissions", "tracks"].includes(newValue)
					? Array.isArray(cache.data[newValue])
						? cache.data[newValue]
						: []
					: [],
				defination: ["commissions", "tracks"].includes(newValue)
					? definations[newValue]
					: prevState.defination,
				service: ["commissions", "tracks"].includes(newValue)
					? ApiService.getContextRequests(definations[newValue])
					: prevState.service,
				rendered_contexts: rendered_contexts,
				context_entry_type: ["commissions", "tracks"].includes(newValue)
					? definations[newValue].views.listing.googlemapview.type
					: "clients_positions",
			};
		});
	};
	...
	handleToggleContextRender = (context, renderContext) => {
		...
		this.setState(
			prevState => {
				let renderedContextIndex =
					prevState.rendered_contexts.indexOf(context);
				return {
					rendered_contexts: prevState.rendered_contexts.includes(
						context
					)
						? prevState.rendered_contexts.removeAtIndex(
								renderedContextIndex
						  )
						: prevState.rendered_contexts.concat([context]),
				};
			},
			async () => {
				if (definations[context] && renderContext) {
					apiCallRequest(definations[context].name, {
						...
						type: "records",
						params: JSON.isJSON(this.state.contexts_props[context])
							? this.state.contexts_props[context].query
							: {
									page: 1,
									pagination: app.preferences.data.pagination,
									populate: 1,
						...
						.then(res => {
							...
							if (
								definations[context] &&
								Function.isFunction(
									definations[context].views.listing
										.googlemapview.resolveData
								) &&
								Array.isArray(data)
							) {
								definations[context].views.listing.googlemapview
									.resolveData(data, true)
									.then(resolve => {
										if (this.isMounted()) {
											this.setState(prevState => ({
												context_entry_type:
													definations[context].views
														.listing.googlemapview
														.type,
												context_entry:
													data.length > 0
														? 0
											...
											this.state.context_entry_type =
												definations[
													context
												].views.listing.googlemapview.type;
											this.state.context_entry =
												data.length > 0 ? 0 : undefined;
				...
				if (entry.positions) {
					if (Array.isArray(entry.positions)) {
						if (JSON.isJSON(entry.positions[0])) {
							googleMap.panTo({
								lat: entry.positions[0].latitude,
								lng: entry.positions[0].longitude,
							});
						}
					}
				}
	...
	render() {
		...
		rendered_contexts.map(rendered_context => {
			if (
				definations[rendered_context] &&
				rendered_context in contexts_props
			) {
				if (
					contexts_props[rendered_context].element === "marker" &&
					Array.isArray(contexts_props[rendered_context].elements)
				) {
					markers = markers.concat(
						contexts_props[rendered_context].elements
					);
				} else if (
					contexts_props[rendered_context].element === "circle" &&
					Array.isArray(contexts_props[rendered_context].elements)
				) {
					...
					contexts_props[rendered_context].element === "polyline" &&
					Array.isArray(contexts_props[rendered_context].elements)
				) {
					...
					title={
						this.props.context
							? definations[this.props.context]
								? definations[this.props.context].label + " Map"
								: "Map"
							: "Map Overview"
					}
					subheader="Click on any context element in the map for more infomation on the context "
					sx={{
					...
					action={
						<div>
							...
							{Array.isArray(actions) &&
								actions.map(
									({ onClick, icon, ...rest }, cursor) => (
										<IconButton
											aria-label="Action"
											className={
												"mx-1 inverse-text translucent "
											}
											onClick={onClick}
											style={{
												backgroundColor:
							...
							<IconButton
								...
								color={"secondary"}
								className={"mx-1"}
								title="Toggle Map Overview Drawer Menu"
				...
				<CardContent className="p-0 m-0 overflow-x-hidden relative">
					<GridContainer className="p-0 m-0">
						<GridItem xs={12} className={`m-0 p-0 flex relative`}>
							<div
								...
								style={{
									minHeight: device.window_size.height,
									backgroundColor:
								...
								<GoogleMap
									...
									showClientsPositions={rendered_contexts.includes(
										"clients_positions"
									)}
					...
					<Drawer
						...
						{context === "clients_positions" &&
							Object.size(clients_positions) === 0 && (
								<GridContainer>
									<GridItem
										xs={12}
						...
						{context === "clients_positions" && (
							<List className={`p-0`}>
								{Object.entries(clients_positions).map(
										...
										if (client.user && show_entry) {
											return (
												<ListItem
													...
													onClick={this.handleClientListItemClick(
														socketId,
														client
													)}
													key={"client-" + socketId}>
													<ListItemAvatar>
														<Avatar
															src={this.getclientPositionMarkerIcon(
																client.user
															)}
														/>
													</ListItemAvatar>
							...
							<Listings
								...
								onLoadData={(loadedData, query) => {
									if (
										...
										definations[
											context
										].views.listing.googlemapview
											.resolveData(loadedData, true)
											.then(resolve => {
												this.setState(prevState => {
													let renderedContextIndex =
														prevState.rendered_contexts.indexOf(
															context
														);
													let rendered_contexts =
														renderedContextIndex !==
														-1
															? prevState.rendered_contexts
															: prevState.rendered_contexts.concat(
																	[context]
															  );
						...
						{Array.isArray(contexts) &&
							contexts.map((context_entry_name, cursor) => (
								<GridItem
									...
									key={"context_entry_switch-" + cursor}>
									<SwitchInput
										label={
											context_entry_name ===
											"clients_positions"
										...
										value={this.state.rendered_contexts.includes(
											context_entry_name
										)}
										onChange={new_value => {
											this.handleToggleContextRender(
