	...
	return (
		...
		>
							...
							active_conversation.participants[0].avatar ? (
								<Avatar
									...
									src={ApiService.getAttachmentFileUrl(
										active_conversation.participants[0]
											.avatar
									)}
								/>
							) : active_conversation.started_by &&
							  active_conversation.started_by.avatar ? (
								<Avatar
									className={"mr-6 w-6 h-6"}
									src={ApiService.getAttachmentFileUrl(
										active_conversation.started_by.avatar
								...
								? (auth.user._id ===
								  ...
								  active_conversation.participants.length > 0
									...
									? active_conversation.started_by
											.first_name +
									  " " +
									  active_conversation.started_by.last_name
									: ""
								: active_conversation.type == "group"
				...
				classes={{
					paper:
						window_size.width >= 768
							? classes?.drawerPaper
							: classes?.temporaryDrawerPaper,
				}}
			>
				<div
					className={
						"w-full flex flex-row items-center h-16 secondary"
					}
				>
							...
							onLoadData={(loadedData, loadedQuery) => {
								if (
									...
									loadedData.map(entry => {
										if (
											entry._id === loadedQuery._id ||
											entry.email_address ===
												loadedQuery.email_address
										) {
											handleNewChat(entry);
										}
									});
			...
			{!active_conversation && (
				<GridItem
					md={12}
					className={"flex flex-col relative min-h-full p-0"}
				>
					<Paper elevation={0} square className={classes?.paper}>
						<ScrollBars
							className={
								"overflow-x-hidden overflow-y-scroll  pb-16"
							}
							onYReachStart={event => {
								//
							}}
								...
								(!Array.isArray(conversations) ||
									...
									<GridContainer>
										<GridItem
											md={12}
											className={
												"flex flex-row items-center relative p-0 px-4 my-4"
											}
										>
											<Skeleton
												variant="circle"
								...
								(!Array.isArray(conversations) ||
									...
									<GridContainer>
										<GridItem
											md={12}
											className={
												"flex flex-col items-center relative p-0 px-4 my-4"
											}
										>
											<Typography
												variant="subtitle1"
								...
								<List className={classes?.list}>
									{conversations.map((chat, index) => {
										if (
											index === 0 ||
											(index > 0 &&
												conversations[index - 1]._id !==
													conversations[index]._id)
										) {
											let primaryText = "";
											let chatUser = false;
											let avatar = false;
			...
			{active_conversation && (
				...
				>
					<GridItem
						md={12}
						className="h-full p-0 flex flex-col relative"
					>
						{!["audio", "file", "video", "image"].includes(
							draft.type
						) && (
							<ScrollBars
								className={
									"flex-grow " + classes?.chatScrollWrapper
								}
								className={
									"overflow-x-hidden overflow-y-scroll h-4/5  pb-8"
								}
								style={{
									flex: 10,
									backgroundColor:
									...
									if (element) {
										if (active_conversation.state) {
											if (
												...
												active_conversation.state
													.pages > 0 &&
												active_conversation.state.page >
													0 &&
												active_conversation.state.page <
													active_conversation.state
														.pages
							...
							>
								<GridContainer className="px-4 relative">
									...
									{active_conversation_messages.map(
										(activeChatMessage, cursor) => {
												...
												if (cursor > 0) {
													if (
														...
														(activeChatMessage
															.conversation
															._id !==
															active_conversation._id &&
															activeChatMessage.conversation !==
																active_conversation._id)
													) {
														show_message = false;
													}
															...
															className={
																...
																activeChatMessage.sender ===
																	auth.user
																		._id
																	? "bg-green-200 " +
																	  classes?.chatBubbleLocal
																	: "bg-white " +
																	  classes?.chatBubbleExternal)
															}
														...
														>
															...
															{!message_deleted &&
																[
																	...
																	activeChatMessage.type
																) &&
																Array.isArray(
																	activeChatMessage.attachments
																) && (
																	<div
																		className={
																	...
																	>
																		{activeChatMessage.attachments.map(
																			(
																				attachment,
																				cursor
																			) => (
																				<div
																					className={
																						"p-2 w-11/12 border border-gray-400 rounded"
																					}
																					key={
																						"attachment-" +
																						cursor
															...
															{!message_deleted && (
																<div
																	className={
																		"flex flex-row items-center text-sm w-full"
																	}
																	style={{
																		color: theme
																			.palette
																...
																>
																	{activeChatMessage.created_on && (
																		...
																		>
																			{activeChatMessage.created_on instanceof
																			Date
									...
									{(Array.isArray(active_conversation.typing)
										? active_conversation.typing.length > 0
										: false) && (
										<GridItem
											xs={12}
											className={"flex p-0 py-1 flex-row"}
										>
											...
											<div
												className={
													"p-2 mx-4  flex flex-col px-4  rounded-full"
												}
											>
												<div
													className={
														classes?.typing_loader
													}
												></div>
											</div>
										</GridItem>
						...
						{(auth.user.isAdmin ||
							...
							contactactable_contacts_ids.includes(
								active_conversation.recipients[0]
							) ||
							active_conversation.type !== "individual") && (
							<GridContainer
								className={
							...
							>
								{draft.reply_for && (
									<GridItem
										xs={12}
										className={
											"flex flex-row items-center bg-green-200"
										}
									>
								...
								{["audio", "file", "video", "image"].includes(
									...
									<GridItem
										xs={12}
										className={
											"flex flex-grow flex-row items-center bg-grey-200"
										}
									>
										<FileInput
											variant="outlined"
								...
								{attachmentDialOpen && (
									<GridItem
										xs={12}
										className={
											"flex flex-row items-center justify-evenly bg-grey-200"
										}
									>
										{attachmentActions.map(action => (
											<Fab
												...
												onClick={() => {
													setAttachmentDialOpen(
														false
													);
													handleOnChangeDraftType(
														action.name
													);
												}}
												style={{
						...
						<MenuItem
							onClick={event => {
								...
								updateMessage({
									...
									deletions:
										active_conversation.recipients.concat([
											active_conversation.owner,
										]),
								});
