...
const Conversation = props => {
	...
	const getTextHeight = (text, font) => {
		...
		const wrappedLinesArr = getCanvasContextWrappedLines(
			context,
			text,
			maxWidth
		)
	...
	const messageNeedsDateHeader = useCallback((index) => {
	...
	const getHeights = useCallback(() => {
		...
		if (Array.isArray(data?.messages)) {
			heights = data.messages.reduce((currentHeights, message, messageIndex) => {
				let messageHeight =
					lineHeightRef.current * 4
				if (String.isString(message.content) &&
					...
					messageHeight =
						messageHeight +
						contentTextHeight
					console.log("contentTextHeight", contentTextHeight)
	...
	useDidMount(() => {
		if (!!containerRef.current) {
			...
			lineHeightRef.current = getTextWidth(
				"A",
				fontSizeRef.current
			) * 1.75
			...
			setState({
				...
				if (
					!!listRef.current &&
					Function.isFunction(listRef.current.scrollToPosition) &&
	...
	return (
		<Box
			className={`relative h-screen flex flex-col bg-transparent text-base ${className} `}
			ref={containerRef}
		>
