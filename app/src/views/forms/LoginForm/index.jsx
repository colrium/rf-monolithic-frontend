...
const LoginForm = (props) => {
	const { auth, onLogin, login, history, app: { settings: { auth: OAuth2_enabled } } } = props;
	...
	const loginBtnClassname = classNames({
		...
		[classes?.loginSubmitting]: state.loginSubmitting,
		[classes?.loginSuccess]: state.loginsuccess,
		[classes?.loginError]: state.loginerror
	});
	...
	const onClickForgotPassword = () => {
		setState(state => ({ forgotPassword: !state.forgotPassword, resettingPassword: state.resettingPassword ? false : false }));
	}
	...
	const handleLoginSubmit = async (event) => {
		...
		//
		const { email, forgotPassword, resettingPassword, reset_code, password, repeat_password } = getState();
		setState(state => ({
			loginerror: false,
			loginsuccess: false,
			submitting: true,
		...
		if (forgotPassword) {
			...
			login(formData).then(({ access_token, profile }) => {
				setState(state => ({
					submitting: false,
					loginerror: false,
					loginsuccess: true,
				...
				handleOnUserLogin({ access_token: access_token, profile: profile });
			}).catch(err => {
				setState(state => ({
					submitting: false,
					loginerror: true,
	...
	return (
		<div className={classes?.root}>
			<GridContainer className="p-0">
				...
				>
					<GridContainer className={classes?.loginContainer}>
						{(!state.resettingPassword && (state.resettingPassword || !state.resettingPasswordWithUrl)) && (
							<GridItem
								xs={12}
								sm={12}
								md={12}
						...
						{(state.resettingPassword && !state.resettingPasswordWithUrl) && <GridItem
							xs={12}
							sm={12}
							md={12}
							className={classes?.inputContainer}
						...
						{(!state.forgotPassword || state.resettingPassword) && (
							...
							>
								<TextInput
									...
									label={state.resettingPassword ? "Enter new password" : "Password"}
									InputProps={{
										classes: {
											input: "inverse",
											adornedEnd: "inverse",
									...
									validate
									variant="filled"
									excludeValidation={state.resettingPassword ? [] : ["password"]}
									disabled={state.submitting}
								/>
							</GridItem>
						)}
						...
						{state.resettingPassword && (
							...
							>
								<TextInput
									...
									label={state.resettingPassword ? "Repeat new password" : "Repeat Password"}
									variant="filled"
									InputProps={{
										classes: {
											input: "inverse",
									...
									required
									validate
									validator={(value) => { return value === state.password ? false : "Passwords dont match" }}
									fullWidth
									disabled={state.submitting}
								/>
							</GridItem>
						...
						<GridContainer>
							...
							<GridItem xs={12} md={6} className="m-0 p-0">
								...
								{!state.submitting && <GridContainer className={"p-0"}>
									<GridItem xs={12} className={"flex flex-col items-end justify-end"}>
										<Button
											...
											type="submit"
										>
											{state.forgotPassword ? " Send password reset code" : (state.resettingPassword ? "Reset Password" : "Login")}
										</Button>
									</GridItem>
						...
						<GridContainer>
							...
							<GridItem md={6} className="p-0 flex flex-col items-center md:justify-end sm:justify-center">
								...
								{!state.forgotPassword && !state.resettingPassword && <Link to={"/jobs".toUriWithLandingPagePrefix()} className="transparent">
									<Button
						...
						{OAuth2_enabled && <GridContainer className="items-center justify-center p-0">
							<Typography
								className="p-0"
								variant="body1"
								fullWidth
						...
						{OAuth2_enabled && <GridContainer className="items-center justify-center">
							<OAuth onOAuthSuccess={handleOnOAuthSuccess} />
						</GridContainer>}
					</GridContainer>
				</form>
			</GridContainer>
			...
			>
				<Alert onClose={handleSnackbarClose} severity={state.loginerror ? "error" : state.loginsuccess ? "success" : "error"} sx={{ width: '100%' }}>
...
export default compose(connect(mapStateToProps, { login }), withRouter)(LoginForm);
