...
class Step extends React.Component {
	...
	getGatewayData(){
		...
		paymentsService.gatewayData(data).then((res) => {
			this.setState(state => ({ gatewayData: res.body.data,load_error: false, loading: false }));
		}).catch((err)=>{
			console.error("Checkout loadData Error", err);
			this.setState(state => ({ gatewayData:{}, load_error: err, loading: false }));
		});
	}
	...
	render() {
		...
		return (
			<GridContainer className="p-0 m-0">
				<GridContainer>
					<GridItem xs={12}>
						<Typography variant="h3" className="primary_text">Checkout</Typography>	
					</GridItem>
				</GridContainer>
							...
							<div className="panel-right w-full md:w-1/3 secondary text-white rounded-r">
								<div className="p-10">
									...
									<div className="mb-4 text-6xl">
										...
										<span className="font-light lh-fix">99</span>
									</div>
									...
									<div className="mb-10 pb-2">
										<div className="mb-2">Need <span className="font-bold">20</span> clients?</div>
										<a href="#" className="text-white font-bold border-b-2 border-solid border-blue-light">Switch to the Agency plan.</a>
									</div>
									...
									<div className="testimonial pt-10 text-lg italic leading-normal mb-4">
										FTP is horrible to deal with. <span className="font-bold">WP Pusher</span> solves that issue, and the customer support is awesome!
									</div>
									<div className="flex items-center">
										<img src="https://s3-us-west-2.amazonaws.com/s.cdpn.io/195612/wppusher_avatar.png" alt="avatar" className="rounded-full mr-2" />
										<div className="text-lg font-bold">Kirk Beard</div>
									</div>
								</div>
							</div>
						</div>
				...
				<GridContainer className="p-4">
					<GridItem xs={12} md={6} className="flex justify-start">
						<form action={this.state.gatewayData.ipn} method="post">
							{Object.entries(this.state.gatewayData).map(([name, value], cursor)=>(
					...
					<GridItem xs={12} md={6} className="flex justify-end">
						<Button onClick={onCancel} color="inverse" round > Go back </Button>
					</GridItem>
				</GridContainer>
