		...
		const subscription = observer$.subscribe(formData => {
			if (JSON.isEmpty(formData.errors)) {
				EventRegister.emit("change-preferences", {
					notifications: { ...preferences, ...formData.values },
				})
			}
		})
			...
			<Grid container className="px-8">
				...
				<Grid container className="px-0">
					...
					<Grid item xs={12} md={4} className="mb-1">
						<Checkbox name="all.push" label="Push" onChange={toggleAllOthers("push")} />
					</Grid>

					<Grid item xs={12} md={4} className="mb-1">
						<Checkbox name="all.sms" label="SMS" onChange={toggleAllOthers("sms")} />
					</Grid>

					<Grid item xs={12} md={4} className="mb-1">
						<Checkbox name="all.email" label="Email" onChange={toggleAllOthers("email")} />
					</Grid>
				</Grid>
