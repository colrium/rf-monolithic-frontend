...
import PasswordWidget from "views/widgets/Preferences/Password";
import DashboardWidget from "views/widgets/Preferences/Dashboard";
	...
	constructor(props) {
		...
		for (var i = 0; i < nav.entries.length; i++) {
			if (nav.entries[i].name === "preferences") {
				this.state.active_tab = nav.entries[i].view? nav.entries[i].view : "dashboard";
			}			
		}
		...
		return (
			<GridContainer className="px-2 pt-8">
				<GridItem xs={12} md={tabs_orientation === "vertical"? 3 : 12} lg={tabs_orientation === "vertical"? 2 : 12}>
					...
					>
						{Object.entries(this.state.tabs).map(([name, value], cursor) => (
							<Tab label={value} value={name} id={"preferences-tab-"+name} aria-controls={"preferences-tabpanel-"+name} key={"preferences-tab-"+cursor}/>
						))}
					</Tabs>
				...
				<GridItem xs={12} md={tabs_orientation === "vertical"? 9 : 12} lg={tabs_orientation === "vertical"? 10 : 12} >
					{ Object.entries(this.state.tabs).map(([name, value], cursor) => (
						<TabPanel value={this.state.active_tab} index={name} key={"preferences-tabpanel-"+cursor}>
							{name === "password" && <PasswordWidget /> }
							{name === "dashboard" && <DashboardWidget /> }						
						</TabPanel>
					))}
				</GridItem>
			</GridContainer>
