...
class Page extends React.Component {
	...
	componentDidMount() {
		...
		if (appendNavHistory && location && this.defination) {
			appendNavHistory({
				...
				title: Function.isFunction(this.defination.label) ? this.defination.label(auth.user).singularize() : this.defination.label.singularize(),
				view: null,
	...
	getRecord() {
		this.service .getRecordById(this.id, { p: 1 })
			.then(res => {
	...
	render() {
		...
		const forbidden = this.state.loading ? false : (JSON.isJSON(this.state.record) ? this.defination.access.actions.view_single.restricted(auth.user) : true);
		return (
										...
										<GridContainer>
											...
											{!forbidden && <GridContainer className="p-0 m-0">
												<GridItem className="p-0 m-0" xs={12}>
													{this.state.record && <RecordView defination={this.defination} service={this.service} record={this.state.record} />}
												</GridItem>
											</GridContainer>}
										</GridContainer>
									)}
