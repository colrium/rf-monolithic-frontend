...
import { FileInput, TextInput } from "components/FormInputs";
...
const AlwaysScrollToBottom = React.forwardRef((props, ref) => {
	...
	useEffect(() => {
		/*SocketIO.on("message-sent", scrollIntoView);
		SocketIO.on("new-message", scrollIntoView);*/
		//scrollIntoView();
		return () => {
			/*SocketIO.off("message-sent", scrollIntoView);
			SocketIO.off("new-message", scrollIntoView);*/
		}
	}, [])
	return <div ref={ref} />
})
...
function Chat(props) {
	...
	const handleArchiveConversation = conversation => {
		...
		SocketIO.emit("archive-conversation", {
	...
	const handleDeleteConversation = conversation => {
		...
		SocketIO.emit("delete-conversation-for-user", {
			conversation: conversation,
			user: auth.user,
		})
	}
	...
	const getConversationMessages = (conversation, query = { desc: "created_on", p: "1", page: 1, pagination: 10 }) => {
		if (conversation) {
			...
			//SocketIO.emit("get-conversation-messages", {conversation: conversation._id, user: auth.user});
		} else {
			setLoadingActiveChatMessages(false)
		}
	}
	...
	const handleOnSendMessage = (messageToSend, conversation) => {
		if (conversation) {
			...
			if (
				...
				(["audio", "file", "video", "image"].includes(newMessage.type) &&
					(Array.isArray(newMessage.attachments) ? newMessage.attachments.length > 0 : false))
			) {
		...
		if (event.key === "Enter" && conversation) {
			...
			if (
				...
				(["audio", "file", "video", "image"].includes(draft.type) &&
					(Array.isArray(draft.attachments) ? draft.attachments.length > 0 : false))
			) {
	...
	useDidMount(() => {
		SocketIO.on("presence-changed", handlePresenceChanged)
		SocketIO.on("user-changed-presence", handlePresenceChanged)
		//SocketIO.on("message-typing-started", handleOnUserStartedTypingMessage);
		//SocketIO.on("message-typing-stopped", handleOnUserStoppedTypingMessage);
		//SocketIO.on("conversation-messages", handleOnConversationMessages);
		/*SocketIO.on("new-message", handleOnNewMessage);
		SocketIO.on("message-sent", handleOnMessageSent);*/
		SocketIO.on("messages-deleted-for-user", handleOnMessageDeletedForUser)
		SocketIO.on("messages-deleted-for-all", handleOnMessageDeletedForAll)
		SocketIO.on("message-marked-as-received", handleOnMessageStateChangedBySocketAction)
		SocketIO.on("message-received", handleOnMessageStateChangedBySocketAction)
		SocketIO.on("message-marked-as-read", handleOnMessageStateChangedBySocketAction)
		SocketIO.on("message-read", handleOnMessageStateChangedBySocketAction)
		SocketIO.on("delete-conversation-for-user-error", handleSocketActionError)
		SocketIO.on("delete-message-for-user-error", handleSocketActionError)
		SocketIO.on("delete-message-for-all-error", handleSocketActionError)
	})

	useWillUnmount(() => {
		SocketIO.off("presence-changed", handlePresenceChanged)
		SocketIO.off("user-changed-presence", handlePresenceChanged)
		//SocketIO.off("conversation-messages", handleOnConversationMessages);
		/*SocketIO.off("new-message", handleOnNewMessage);
		SocketIO.off("message-sent", handleOnMessageSent);*/
		SocketIO.off("messages-deleted-for-user", handleOnMessageDeletedForUser)
		SocketIO.off("messages-deleted-for-all", handleOnMessageDeletedForAll)
		SocketIO.off("delete-conversation-for-user-error", handleSocketActionError)
		SocketIO.off("message-marked-as-received", handleOnMessageStateChangedBySocketAction)
		SocketIO.off("message-received", handleOnMessageStateChangedBySocketAction)
		SocketIO.off("message-marked-as-read", handleOnMessageStateChangedBySocketAction)
		SocketIO.off("message-read", handleOnMessageStateChangedBySocketAction)
		SocketIO.off("delete-message-for-user-error", handleSocketActionError)
		SocketIO.off("delete-message-for-all-error", handleSocketActionError)
		SocketIO.off("create-conversation-error", handleSocketActionError)
	})
	...
	return (
		...
		>
				...
				<Toolbar>
					...
					{active_conversation &&
						(active_conversation.type == "individual" ? (
							(auth.user?._id === active_conversation.owner._id || auth.user?._id === active_conversation.owner) &&
							...
							active_conversation.participants[0].avatar ? (
								<Avatar
									...
									src={ApiService.getAttachmentFileUrl(active_conversation.participants[0].avatar)}
								/>
							) : active_conversation.started_by && active_conversation.started_by.avatar ? (
								<Avatar
									className={"mr-6 w-6 h-6"}
									src={ApiService.getAttachmentFileUrl(active_conversation.started_by.avatar)}
								/>
							) : (
						...
						) : active_conversation.group_avatar ? (
							<Avatar className={"mr-6"} src={ApiService.getAttachmentFileUrl(active_conversation.group_avatar)} />
						) : (
							<Avatar className={"mr-6 w-6 h-6 bg-transparent accent-text"}>
								{" "}
								{active_conversation.type === "group" ? <PeopleIcon /> : <PersonIcon />}
							</Avatar>
						))}
					{active_conversation && (
						<Typography variant="h6" className={"capitalize flex-grow"}>
							{active_conversation.type == "individual"
								? (auth.user?._id === active_conversation.owner._id || auth.user?._id === active_conversation.owner) &&
								  Array.isArray(active_conversation.participants) &&
								  active_conversation.participants.length > 0
									? active_conversation.participants[0].first_name + " " + active_conversation.participants[0].last_name
									: active_conversation.started_by
									? active_conversation.started_by.first_name + " " + active_conversation.started_by.last_name
									: ""
								: active_conversation.type == "group"
								? active_conversation.group_name
								: "Realfield"}
						</Typography>
					)}
					...
					{!active_conversation && !contactsDrawerOpen && (
						<IconButton
			...
			<SwipeableDrawer
				className={classNames({
					"w-40": true /*, "hidden": !contactsDrawerOpen*/,
				})}
			...
			{!active_conversation && (
				<GridItem md={12} className={"flex flex-col relative min-h-full p-0"}>
					<Paper elevation={0} square className={classes?.paper}>
						...
						>
							{fetching_inbox &&
								(!Array.isArray(conversations) || (Array.isArray(conversations) && conversations.length === 0)) && (
									<GridContainer>
									...
									{conversations.map((chat, index) => {
										if (index === 0 || (index > 0 && conversations[index - 1]._id !== conversations[index]._id)) {
											...
											if (chat.state && chat.state.total > 0 && chat.state.last_message) {
												...
												if (
													chat.state.last_message.sender === auth.user?._id ||
													chat.state.last_message.sender._id === auth.user?._id
												) {
												...
												if (
													(chat.state.last_message.state === "deleted-for-sender" &&
														(chat.state.last_message.sender._id === auth.user?._id ||
															chat.state.last_message.sender === auth.user._id)) ||
													chat.state.last_message.state === "deleted-for-all"
												) {
											...
											return (
													...
													style={{
														background:
															contextMenu.type === "conversation"
																? contextMenu.entry._id === chat._id
												...
												>
													{chatUser && (
														<ListItemAvatar>
															<Badge
																...
																classes={{
																	dot:
																		chatUser.presence === "online"
																			? "bg-green-600"
																			: chatUser.presence == "away"
																			? "bg-orange-500"
																			: "bg-gray-500",
																}}
															>
																{chatUser.avatar && (
																	<Avatar
																		...
																		src={ApiService.getAttachmentFileUrl(chatUser.avatar)}
																	/>
																)}
																{!chatUser.avatar && (
															...
															<div className="w-full flex flex-row">
																...
																{(!Array.isArray(chat.typing) || chat.typing.length === 0) && (
																	<div className="w-full flex flex-row items-center">
																		...
																		{last_message_deleted && (
																			<div
																				className={"flex flex-row items-center flex-initial"}
																				style={{
																		...
																		{!last_message_deleted && chat.state && chat.state.last_message && (
																			<div
																			...
																			>
																				{chat.state.last_message.state === "pending" &&
																					(chat.state.last_message.sender
																						? chat.state.last_message.sender ===
																								auth.user._id ||
																						  chat.state.last_message.sender._id ===
																								auth.user._id
																						: false) && (
																						<ScheduleIcon
																				...
																				{chat.state.last_message.state === "sent" &&
																					chat.state.last_message.sender === auth.user._id && (
																						<DoneIcon
																							className={"mx-1 text-xs"}
																							fontSize="small"
																				...
																				{(chat.state.last_message.state === "partially-received" ||
																					chat.state.last_message.state === "received") &&
																					(chat.state.last_message.sender
																						? chat.state.last_message.sender ===
																								auth.user._id ||
																						  chat.state.last_message.sender._id ===
																				...
																				{(chat.state.last_message.state === "partially-read" ||
																					chat.state.last_message.state === "read") &&
																					(chat.state.last_message.sender
																						? chat.state.last_message.sender ===
																								auth.user._id ||
																						  chat.state.last_message.sender._id ===
													...
													/>
													{chat.state && chat.state.incoming_unread > 0 && (
			...
			{active_conversation && (
				<Paper square elevation={0} className={"relative flex-grow h-screen overflow-hidden"}>
					<GridItem md={12} className="h-full p-0 flex flex-col relative">
						{!["audio", "file", "video", "image"].includes(draft.type) && (
								...
								onYReachStart={element => {
									if (element) {
										if (active_conversation.state) {
											if (
												!active_conversation.state.loading_messages &&
												active_conversation.state.pages > 0 &&
												active_conversation.state.page > 0 &&
												active_conversation.state.page < active_conversation.state.pages
											) {
												let pagination =
													active_conversation.state.pagination > 0 ? active_conversation.state.pagination : 20
												let nextPage = active_conversation.state.page + 1
												let nextQuery = JSON.isJSON(active_conversation.state.query)
													? active_conversation.state.query
													: {}
									...
									if (element) {
										if (active_conversation.state) {
											if (
												!active_conversation.state.loading_messages &&
												active_conversation.state.pages > 0 &&
												active_conversation.state.page > 1
											) {
												let pagination =
													active_conversation.state.pagination > 0 ? active_conversation.state.pagination : 20
												let nextPage = active_conversation.state.page - 1
							...
							>
								<GridContainer className="px-4 relative">
									{active_conversation.state && active_conversation.state.loading_messages && (
										<div
											style={{
												position: "absolute",
												left: "50%",
									...
									{active_conversation_messages.map((activeChatMessage, cursor) => {
										...
										if (active_conversation_messages[cursor]._id) {
											if (cursor > 0) {
												if (
													active_conversation_messages[cursor]._id ===
														active_conversation_messages[cursor - 1]._id ||
													(activeChatMessage.conversation._id !== active_conversation._id &&
														activeChatMessage.conversation !== active_conversation._id)
												) {
													show_message = false
												}
											} else {
										...
										if (activeChatMessage && show_message) {
											if (
												(activeChatMessage.state === "deleted-for-sender" &&
													(activeChatMessage.sender === auth.user?._id ||
														activeChatMessage.sender._id === auth.user._id)) ||
												activeChatMessage.state === "deleted-for-all"
											) {
											...
											{!activeChatMessage.sender.avatar && <Avatar className={classes?.contactAvatar}>
												<PersonIcon />
													...
													<ViewPortSensor
														className={
															"p-2 mx-4  flex flex-col px-4 " +
															(activeChatMessage.sender._id === auth.user._id ||
															activeChatMessage.sender === auth.user._id
																? "bg-green-200 " + classes?.chatBubbleLocal
																: "bg-white " + classes?.chatBubbleExternal)
														}
															...
															if (inViewport) {
																if (
																	...
																	if (activeChatMessage.state === "sent") {
																		...
																		activeChatMessage.state === "received" ||
																		activeChatMessage.state === "partially-received"
																	) {
													...
													>
														{!message_deleted && (
															...
															>
																{active_conversation.type !== "individual" &&
																	JSON.isJSON(activeChatMessage.sender) && (
																		<Typography
																			variant="body1"
																			className={"flex-grow text-gray-500 font-bold"}
																		>
																			{activeChatMessage.sender.first_name +
																				" " +
																				activeChatMessage.sender.last_name}
																		</Typography>
																	)}
																<IconButton
																	aria-label="Toggle Menu"
																	onClick={handleContextOpen("message", activeChatMessage)}
																	className={""}
																	size="small"
																>
														...
														{!message_deleted &&
															["audio", "file", "video", "image"].includes(activeChatMessage.type) &&
															Array.isArray(activeChatMessage.attachments) && (
																<div className={"flex flex-col items-center cursor-pointer mb-2"}>
																	{activeChatMessage.attachments.map((attachment, cursor) => (
																		<div
																			className={"p-2 w-11/12 border border-gray-400 rounded"}
																			key={"attachment-" + cursor}
																		>
																			{activeChatMessage.type === "image" && (
																				<LazyImage
																					...
																					onClick={e => {
																						e.preventDefault()
																						let win = window.open(
																							ApiService.getAttachmentFileUrl(attachment),
																							"_blank"
																						)
																						win.focus()
																					}}
														...
														{!message_deleted && (
															...
															>
																{activeChatMessage.created_on && (
																	<Typography className={"text-xs flex-grow"}>
																		{activeChatMessage.created_on instanceof Date
																			? activeChatMessage.created_on.toLocaleString()
																			: new Date(activeChatMessage.created_on).toLocaleString()}
																	</Typography>
																)}
																{activeChatMessage.state === "pending" &&
																	(activeChatMessage.sender._id === auth.user._id ||
																		activeChatMessage.sender === auth.user._id) && (
																		<ScheduleIcon className={"mx-2 text-xs"} fontSize="small" />
																	)}
																{activeChatMessage.state === "sent" &&
																	(activeChatMessage.sender._id === auth.user._id ||
																		activeChatMessage.sender === auth.user._id) && (
																		<DoneIcon className={"mx-2 text-xs"} fontSize="small" />
																	)}
																{(activeChatMessage.state === "partially-received" ||
																	activeChatMessage.state === "received") &&
																	(activeChatMessage.sender._id === auth.user._id ||
																		activeChatMessage.sender === auth.user._id) && (
																		<DoneAllIcon className={"mx-2 text-xs"} fontSize="small" />
																	)}
																{(activeChatMessage.state === "partially-read" ||
																	activeChatMessage.state === "read") &&
																	(activeChatMessage.sender._id === auth.user._id ||
																		activeChatMessage.sender === auth.user._id) && (
																		<DoneAllIcon
									...
									{(Array.isArray(active_conversation.typing) ? active_conversation.typing.length > 0 : false) && (
										<GridItem xs={12} className={"flex p-0 py-1 flex-row"}>
						...
						{(auth.user?.isAdmin ||
							contactactable_contacts_ids.includes(active_conversation.owner) ||
							contactactable_contacts_ids.includes(active_conversation.owner._id) ||
							contactactable_contacts_ids.includes(active_conversation.owner) ||
							contactactable_contacts_ids.includes(active_conversation.recipients[0]) ||
							active_conversation.type !== "individual") && (
							<GridContainer
								className={
									["audio", "file", "video", "image"].includes(draft.type)
										? "flex-grow flex flex-col"
										: classes?.chatInputsWrapper
								}
								style={{
									backgroundColor: theme.palette.background.paper,
									top: ["audio", "file", "video", "image"].includes(draft.type) ? 0 : "auto",
								}}
							>
								{draft.reply_for && (
									<GridItem xs={12} className={"flex flex-row items-center bg-green-200"}>
								...
								{["audio", "file", "video", "image"].includes(draft.type) && (
									<GridItem xs={12} className={"flex flex-grow flex-row items-center bg-grey-200"}>
										<FileInput
											variant="outlined"
											value={
												["audio", "file", "video", "image"].includes(draft.type) &&
												(Array.isArray(draft.attachments) ? draft.attachments > 0 : false)
													? draft.attachments[0]
													: undefined
											}
											onChange={newValue => {
								...
								{attachmentDialOpen && (
									<GridItem xs={12} className={"flex flex-row items-center justify-evenly bg-grey-200"}>
										...
										aria-label={
											attachmentDialOpen
												? ["audio", "file", "video", "image"].includes(draft.type) &&
												  (Array.isArray(draft.attachments) ? draft.attachments > 0 : false)
													? "Remove Attachments"
													: "Dismiss File Attachment"
												: "Close File Attachment"
										}
										className={"mx-2"}
									>
										{!attachmentDialOpen ? (
						...
						<MenuItem
							onClick={event => {
								...
								updateMessage({
									...
									deletions: active_conversation.recipients.concat([active_conversation.owner]),
								})
