	...
	render() {
		return (
			<GridContainer className="p-0 m-0">
				{ this.state.loading && <GridContainer className="flex-10">
					<GridItem xs={12} md={4} lg={3}>
						<div className="bg-white shadow-xl rounded-lg flex flex-col overflow-hidden">
							<Skeleton variant="rect" width={"100%"} height={224} />
							<Skeleton variant="text" width={"95%"} className="mt-2 mx-auto" />
							<Skeleton variant="rect" width={"30%"} height={50} className="mt-2 ml-4"/>
							<Skeleton variant="text" width={"95%"} className="mt-2 mx-auto" />
							<Skeleton variant="text" width={"95%"} className="mt-6 mx-auto" />
							<Skeleton variant="rect" width={"80%"} height={34} className="mt-8 mb-6 mx-auto rounded"/>
						</div>
					</GridItem>
					<GridItem xs={12} md={4} lg={3}>
						<div className="bg-white shadow-xl rounded-lg flex flex-col overflow-hidden">
				...
				{ !this.state.loading && Array.isArray(this.state.retailitems) && <GridContainer className="p-4 m-0">
						{this.state.retailitems.map((retailitem, cursor) => (
							retailitem.available && <GridItem xs={12} xs={12} md={4} lg={3} className="flex flex-col" key={retailitem._id}>
								<div className="bg-white shadow-xl rounded-lg overflow-hidden">
									...
									<div className="p-4">
										...
										{ retailitem.cost > 0 && JSON.isJSON(retailitem.currency) && <p className="text-3xl text-gray-900 my-4">
											{retailitem.currency.html_symbol +" "+retailitem.cost }
										</p> }
										<p className="truncate tracking-wide text-sm  text-gray-500">
											{retailitem.description? retailitem.description : "No description available"}
										</p>
									</div>
									...
									<div className="px-4 pt-3 pb-4 border-t border-gray-300 bg-gray-100">
										<Button className="w-full" color="primary" onClick={this.handleOnItemSelect(retailitem)}>View</Button>
									</div>
								</div>
							</GridItem> ))}
					</GridContainer>}
