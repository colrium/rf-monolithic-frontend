...
import { removeFromCart, setCartNote, setOrder, emptyCart, setCheckoutData, closeDialog, openDialog } from "state/actions";
import { attachments as AttachmentsService, payments as PaymentsService, orders as OrdersService } from "services";
...
function Widget(props) {
	...
	let { cart, order, auth, sockets, removeFromCart, setCartNote, setOrder, emptyCart, setCheckoutData, closeDialog, openDialog, onProceedToCheckout, className } = state;
	...
	function proceedToCheckout(){
		if (auth.isAuthenticated) {
			...
			if (order) {
				...
				PaymentsService.gatewayData(gateway_params_data).then(res => {
						setLoading(false);
				...
				OrdersService.makeOrder(make_order_data).then(res => {
	...
	function setOrderNote(note){
		if (order) {
			...
			OrdersService.update(order._id, updatedOrder).then(res => {
	...
	if (cart.entries.length > 0) {
		return (
			<GridContainer className={"px-4"+(className? (" "+className) : "" )}>
				<GridItem xs={12} sm={12} className="mb-4">
					...
					<Button color="warning" className="mt-2 w-auto" onClick={confirmEmptyCart} right simple> Remove all </Button>
				</GridItem>
						...
						{cart.entries.map((entry, index) => {
							...
							return (
								<ListItem alignItems="flex-start" className="inverse mb-2 rounded" key={"cart-item-"+index}>
									<ListItemAvatar>
										<Avatar>
											<LazyImage alt={item.name} src={item.featured_image? AttachmentsService.getAttachmentFileUrl(item.featured_image) : LogoChevron} />
										</Avatar>
									</ListItemAvatar>
									<ListItemText
										primary={item.name}
										secondary={
											<GridContainer className="p-0">
												<GridItem xs={12} className="m-0 p-0">
													<Typography component="span" variant="body2" className="w-full " color="grey" > {item.description} </Typography>
												</GridItem>
												<GridItem xs={12} className="m-0 mt-2 p-0">
													<Typography component="span" variant="body2" className="inline" color="default" > Cost: { item.cost } </Typography>
												</GridItem>
												<GridItem xs={12} className="m-0 mt-2 p-0">
													<Typography component="span" variant="body2" className="inline" color="default" > Options: { entry.options_cost } </Typography>
												</GridItem>
												<GridItem xs={12} className="m-0 mt-2 p-0">
													<Button color="warning" simple onClick={event=>confirmRemoveFromCart(entry)} disabled={loading}> Remove </Button>
												</GridItem>
											</GridContainer>
										}
									/>
				...
				<GridItem xs={12} className="p-2">
					<GridContainer className="p-0">
						...
						<GridItem xs={12} sm={12} md={6} className="mb-4 flex flex-col">
							<Typography component="span" variant="h2" className="text-right" color="default" >
								<span className="text-gray-500 mr-2 text-base">{cart.currency}</span>
								<span className="text-3xl">{cart.total}</span>
							</Typography>

							<Typography component="span" variant="h4" className="text-right" color="default" >
					...
					<GridContainer className="p-0">
						<GridItem xs={12} sm={12} className="mb-2">
							<Button color="primary" className=" w-auto" onClick={e => { proceedToCheckout()}} right> Proceed to Checkout </Button>
						</GridItem>
						<GridItem xs={12} sm={12} className="mb-4">
							<Typography component="span" variant="body2" className="w-full text-right text-gray-700" > Shipping & taxes calculated at checkout </Typography>
						</GridItem>
					</GridContainer>
		...
		return (
			<GridContainer className={"px-4"+(className? (" "+className) : "" )}>				
				...
				<GridItem xs={12} className="p-0 m-0">
					<Typography component="span" variant="subtitle1"  className="w-full text-center text-gray-500" > Your Cart is empty </Typography>
				</GridItem>
			</GridContainer>
		);
	}
...
export default withRoot(connect(mapStateToProps, { removeFromCart, setCartNote, setOrder, emptyCart, setCheckoutData, closeDialog, openDialog })(Widget));