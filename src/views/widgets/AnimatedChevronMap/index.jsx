...
const styles = theme => ({
	...
	mapPopup: {
		...
		"&.red": {
			color: colors.hex.inverse,
			background:
				"linear-gradient(to bottom right, rgb(253, 176, 6), rgba(251, 0, 6))",
			boxShadow:
		...
		"&.yellow": {
			color: colors.hex.inverse,
			background:
				"linear-gradient(to bottom right, rgb(204, 205, 9), rgb(230, 114, 8))",
			boxShadow:
				"0 14px 26px -12px rgba(204, 205, 9, 0.42), 0 4px 23px 0px rgba(" +
				colors.rgb.default +
				", 0.12), 0 8px 10px -5px rgba(230, 114, 8, 0.2)",
		},
		"&.purple": {
			color: colors.hex.inverse,
			background:
				"linear-gradient(to bottom right, rgb(38, 0, 83), rgb(251, 0, 192))",
			boxShadow:
				"0 14px 26px -12px rgba(38, 0, 83, 0.42), 0 4px 23px 0px rgba(" +
				colors.rgb.default +
				", 0.12), 0 8px 10px -5px rgba(251, 0, 192, 0.2)",
		},
		"&.default": {
			color: colors.hex.inverse,
...
class AnimatedChevronMap extends React.Component {
	...
	animateMap() {
		...
		var activePointSelector =
			...
			":nth-child(" +
			animations[this.state.activePoint].polygonIndex +
			")";
		...
		if (activePoint != null) {
			...
			var relativePosition = {
				left:
					pointPosition.left -
					mapPosition.left -
					mapPopupPosition.width,
				top: pointPosition.top - mapPosition.top + pointPosition.height,
			};
			that.setState({
				nearbyPoints: relatedPoints,
				popup: {
					position: {
						left: relativePosition.left + pointPosition.width,
	...
	render() {
		...
		return (
			<div className={classes.mapWrapper}>
				<div
					className={classes.mapPopup + " " + this.state.popup.color}
					style={{
						top: this.state.popup.position.top + "px",
						left: this.state.popup.position.left + "px",
						width: this.state.popup.width + "px",
					}}
				>
