...
class ListingView extends React.Component {
	state = {
		...
		value_possibilities: {},
		queryBuilderProps: {
			value: { populate: true, config: { fields: {} } },
		},
	};
	mounted = false;
	...
	constructor(props) {
		...
		Object.entries(defination.scope.columns).map(
			([column_name, column_props], cursor) => {
				if (
					![
						"file",
						"dynamic",
						...
						"slider",
						"multiselect",
					].includes(column_props.input.type)
				) {
		...
		if (JSON.isJSON(query)) {
			...
			Object.entries(query).map(([queryColumn, queryValue]) => {
				if (
					...
					filterableColumns.includes(queryColumn)
				) {
	...
	getSnapshotBeforeUpdate(prevProps, prevState) {
		this.mounted = false;
		return {
			prepareForRenderRequired: !Object.areEqual(
				prevProps.query,
				this.props.query
			),
			loadQueryBuilderPropsRequired:
				!Object.areEqual(prevState.fields, this.state.fields) ||
				!Object.areEqual(
					prevState.value_possibilities,
					this.state.value_possibilities
				),
			paramsChangeApplicationRequired: !Object.areEqual(
				prevState.query,
				this.state.query
			),
		};
	...
	shouldComponentUpdate(nextProps, nextState) {
		let shouldUpdate =
			!Object.areEqual(this.props, nextProps) ||
			!Object.areEqual(this.state, nextState);
		return shouldUpdate;
	}
	...
	handleOnRecordsPerPageChanged = event => {
		const pagination = Number.parseNumber(
			event.target.value,
	...
	async prepareForRender() {
		const { defination, query, view, showViewOptions, showAddBtn, app } =
			this.props;
		if (defination) {
			let all_views = {
			...
			let searchKeyword = "";
			this.evaluateFields()
				.then(({ evaluatedFields, onChangeEffects }) => {
					if (this.mounted) {
						this.setState(
							{
								defination: defination,
								service: ApiService.getContextRequests(
									defination.endpoint
								),
			...
			for (const [name, properties] of Object.entries(columns)) {
				...
				if (properties.input.default) {
					if (Function.isFunction(properties.input.default)) {
						field.input.default = await this.callDefinationMethod(
							properties.input.default
						);
					} else {
				...
				if (Function.isFunction(properties.label)) {
					field.label = await this.callDefinationMethod(
						properties.label
					);
				}
				if (Function.isFunction(properties.input.type)) {
					field.input.type = await this.callDefinationMethod(
						properties.input.type
					);
				}

				if (Function.isFunction(properties.input.default)) {
					field.input.value = await this.callDefinationMethod(
						properties.input.default
					);
				}
				...
				if (Function.isFunction(properties.input.required)) {
					field.input.required = await this.callDefinationMethod(
						properties.input.required
					);
				}
				if (Function.isFunction(properties.input.props)) {
					field.input.props = await this.callDefinationMethod(
				...
				if (properties.restricted) {
					if (Function.isFunction(properties.restricted.input)) {
						field.restricted.input =
							await this.callDefinationMethod(
								properties.restricted.input
							);
				...
				if (properties.reference) {
					if (Function.isFunction(properties.reference)) {
						field.reference = await this.callDefinationMethod(
						...
						if (
							Function.isFunction(properties.reference.resolves)
						) {
							field.reference.resolves =
								await this.callDefinationMethod(
									properties.reference.resolves
								);
						} else {
							field.reference.resolves =
								properties.reference.resolves;
				...
				if (properties.possibilities) {
					if (Function.isFunction(properties.possibilities)) {
						field.possibilities = await this.callDefinationMethod(
				...
				onChangeEffects[name] = async instance => {
					let field_with_effects = JSON.parse(
						JSON.stringify(properties)
					);
					if (properties.input.default) {
						if (Function.isFunction(properties.input.default)) {
							field_with_effects.input.default =
								await instance.callDefinationMethod(
									properties.input.default
								);
						}
					}
					...
					if (Function.isFunction(properties.label)) {
						field_with_effects.label =
							await instance.callDefinationMethod(
								properties.label
							);
					}
					if (Function.isFunction(properties.input.type)) {
						field_with_effects.input.type =
							await instance.callDefinationMethod(
								properties.input.type
							);
					}
					if (Function.isFunction(properties.input.disabled)) {
						field_with_effects.input.disabled =
							await instance.callDefinationMethod(
					...
					if (Function.isFunction(properties.input.value)) {
						field_with_effects.input.value =
							await instance.callDefinationMethod(
								properties.input.value
							);
					}
					if (Function.isFunction(properties.input.required)) {
						field_with_effects.input.required =
							await instance.callDefinationMethod(
								properties.input.required
							);
					}
					if (Function.isFunction(properties.input.props)) {
						field_with_effects.input.props =
							await instance.callDefinationMethod(
								properties.input.props
							);
					}
					if (properties.restricted) {
						if (Function.isFunction(properties.restricted.input)) {
							field_with_effects.restricted.input =
								await instance.callDefinationMethod(
									properties.restricted.input
								);
						} else if (
							Boolean.isBoolean(properties.restricted.input)
						) {
							field_with_effects.restricted.input =
								properties.restricted.input;
						} else {
					...
					if (properties.reference) {
						...
						if (Function.isFunction(properties.reference)) {
							...
							if (JSON.isJSON(field_with_effects.reference)) {
								if (
									Function.isFunction(
										field_with_effects.reference.name
									)
								) {
									field_with_effects.reference.name =
										await instance.callDefinationMethod(
							...
							if (
								Function.isFunction(properties.reference.name)
							) {
								field_with_effects.reference.name =
									await instance.callDefinationMethod(
										properties.reference.name
					...
					if (properties.possibilities) {
						if (Function.isFunction(properties.possibilities)) {
							field_with_effects.possibilities =
								await instance.callDefinationMethod(
									properties.possibilities
								);
						}
	...
	loadFieldValuePosibilities() {
		...
		if (Object.size(this.state.onChangeEffectsFields) > 0) {
			for (let [
				effect_field_name,
				effect_field_properties,
			] of Object.entries(this.state.onChangeEffectsFields)) {
				if (
					JSON.isJSON(effect_field_properties.reference) &&
					!this.state.loading[effect_field_name]
				) {
					fields_to_load[effect_field_name] = effect_field_properties;
					loading_fields[effect_field_name] = true;
				} else if (
					(JSON.isJSON(effect_field_properties.possibilities) &&
						!(
							effect_field_name in this.state.value_possibilities
						)) ||
					!Object.areEqual(
						this.state.value_possibilities[effect_field_name],
						effect_field_properties.possibilities
					)
				) {
					value_possibilities[effect_field_name] =
						effect_field_properties.possibilities;
				} else if (
					...
					for (
						var i = 0;
						i < effect_field_properties.possibilities.length;
						i++
					) {
						possibilities[
							effect_field_properties.possibilities[i]
						] = effect_field_properties.possibilities[i];
					}
			...
			for (let [field_name, field_properties] of Object.entries(
				this.state.fields
			)) {
				if (
					JSON.isJSON(field_properties.reference) &&
					!this.state.loading[field_name]
				) {
					...
					(JSON.isJSON(field_properties.possibilities) &&
						!(field_name in this.state.value_possibilities)) ||
					!Object.areEqual(
						this.state.value_possibilities[field_name],
						field_properties.possibilities
					)
				) {
					value_possibilities[field_name] =
						field_properties.possibilities;
				} else if (
					...
					for (
						var i = 0;
						i < field_properties.possibilities.length;
						i++
					) {
						possibilities[field_properties.possibilities[i]] =
							field_properties.possibilities[i];
					}
		...
		if (Object.size(fields_to_load) > 0) {
			for (let [name, field] of Object.entries(fields_to_load)) {
				...
				if (JSON.isJSON(field.reference)) {
					...
					if (
						String.isString(field.reference.name) &&
						JSON.isJSON(field.reference.service_query)
					) {
						let service_key = field.reference.name;
						let service = ApiService.getContextRequests(
							definations[service_key]?.endpoint
						);
							...
							service
								...
								.then(response => {
									...
									if (resolves.emulation) {
										if (
											resolves.emulation.defination in
											definations
										) {
									...
									if (raw_data.length > 0) {
										...
										for (
											var j = 0;
											j < resolved_data.length;
											j++
										) {
											possibilities[
												resolved_data[j][name].value
											] = resolved_data[j][name].resolve;
										}
									}
									if (this.mounted) {
										this.setState(state => ({
	...
	async setQueryBuilderProps() {
		...
		if (fields) {
			Object.entries(fields).map(([column_name, column_props]) => {
				...
				input_type =
					input_type === "textarea" || input_type === "file"
						? "text"
						: input_type;
				input_type = input_type === "radio" ? "select" : input_type;
				input_type =
					input_type === "slider"
						? column_props.type === "integer"
							? "slider"
							: "select"
						: input_type;
				...
				if (input_type === "slider") {
					...
					query_builder_fields[
						column_name
					].fieldSettings.preferWidgets = ["slider"];
				}

				if (!JSON.isEmpty(value_possibilities[column_name])) {
					let listValues = [];
					Object.entries(value_possibilities[column_name]).map(
						([key, value]) => {
							listValues.push({ value: key, title: value });
						}
					);
					query_builder_fields[column_name].fieldSettings.listValues =
						listValues;
				}
			});
		}
		query_builder_fields["created_on"] = query_builder_fields.created_on
			? query_builder_fields.created_on
			: {
					label: "Created On",
					type: "date",
					valueSources: ["value"],
		...
		if (this.mounted) {
			this.setState(prevState => ({
				queryBuilderProps: {
					format: "mongodb",
					value: {
						populate: prevState.query.p || prevState.query.populate,
						sort:
							this.state.query.s ||
							this.state.query.sort ||
							undefined,
						config: { fields: query_builder_fields },
					},
				},
			}));
		} else {
			this.state.queryBuilderProps = {
				format: "mongodb",
				value: {
					populate: this.state.query.p || this.state.query.populate,
					sort:
						this.state.query.s ||
						this.state.query.sort ||
						"-created_on",
				...
				{showPagination && view != "googlemapview" && view != "tableview" && (
					<GridContainer className="p-0 m-0 mb-4">
						<GridItem
							...
							<TablePagination
								count={
									defination.name in cache.res
										? cache.res[defination.name].count
										: 1
								}
								page={
									defination.name in cache.res
										? cache.res[defination.name].page - 1
										: this.state.page - 1
								}
								rowsPerPage={this.state.query.pagination}
								onPageChange={this.handleOnPageChanged}
								...
								onChangeRowsPerPage={
									this.handleOnRecordsPerPageChanged
								}
