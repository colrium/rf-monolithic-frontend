...
import { withClientPositions, withWindowSize } from "hoc"
...
class GoogleMapOverview extends React.Component {
	...
	constructor(props) {
		super(props)
		const { windowSize, cache, context, contexts, app } = this.props
		...
		this.state = {
			...
			drawerOpen: windowSize.width >= 768,
		}
	}
	...
	handleOnClientPositionChanged(socketId, client) {
		this.setState(state => {
			return {
				clients_positions: state.drawerOpen ? { ...state.clients_positions, [socketId]: client } : state.clients_positions,
			}
		})
	}

	handleOnDrawerNavigationValueChange = (event, newValue) => {
		const { services, cache } = this.props
		this.setState(prevState => {
			let renderedContextIndex = prevState.rendered_contexts.indexOf(newValue)
			let rendered_contexts =
				renderedContextIndex !== -1 ? prevState.rendered_contexts : prevState.rendered_contexts.concat([newValue])
			return {
				context: newValue,
				context_entry: null,
	...
	handleToggleContextRender = (context, renderContext) => {
		const { cache, apiCallRequest, app } = this.props
		this.setState(
			prevState => {
				let renderedContextIndex = prevState.rendered_contexts.indexOf(context)
				return {
					rendered_contexts: prevState.rendered_contexts.includes(context)
						? prevState.rendered_contexts.removeAtIndex(renderedContextIndex)
				...
				if (definations[context] && renderContext) {
					apiCallRequest(definations[context].name, {
						...
						.then(res => {
							...
							if (
								definations[context] &&
								Function.isFunction(definations[context].views.listing.googlemapview.resolveData) &&
								Array.isArray(data)
							) {
		...
		return (
			...
			>
					...
					action={
						<div>
							...
							{Array.isArray(actions) &&
								actions.map(({ onClick, icon, ...rest }, cursor) => (
									<IconButton
										aria-label="Action"
										className={"mx-1 inverse-text translucent "}
										onClick={onClick}
							...
							>
								<Icon path={mdiForwardburger} rotate={this.state.drawerOpen ? 0 : 180} size={0.8} color={"inherit"} />
							</IconButton>
						</div>
					}
				/>
				<CardContent className="p-0 m-0 overflow-x-hidden relative">
					<Grid container className="p-0 m-0">
						<Grid item xs={12} className={`m-0 p-0 flex relative`}>
							<div
								...
								style={{
									minHeight: windowSize.height*0.75,
									backgroundColor: theme.palette.background.paper,
								}}
								ref={this.elementRef}
							>
								<GoogleMap
									mapHeight={windowSize.height}
					...
					<Drawer
						variant={windowSize.width >= 768 ? "persistent" : "temporary"}
						anchor="right"
						sx={{
							[`& .MuiDrawer-paper`]: {
								width: drawerWidth,
						...
						{this.state.multicontexts && (
							...
							>
								<BottomNavigationAction
									...
									label="Users"
									icon={<Icon path={mdiAccountCircleOutline} title="Clients Postions Users" size={1} />}
								/>
					...
					<Grid container className="p-0 m-0">
						{Array.isArray(contexts) &&
							contexts.map((context_entry_name, cursor) => (
								...
								>
									<SwitchInput
										label={
											context_entry_name === "clients_positions"
												? "Users"
												: definations[context_entry_name]
												? definations[context_entry_name].label
												: context_entry_name
										}
						...
						<Grid item xs={12}>
							<Typography variant="body2">
								{this.props.context
									? definations[this.props.context]
										? definations[this.props.context].label + " Map from a glance"
										: "Map from a glance"
									: "Map from a glance"}
							</Typography>
						</Grid>
