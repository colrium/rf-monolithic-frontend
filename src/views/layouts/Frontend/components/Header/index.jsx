...
function Header(props) {
	...
	let cart_items_count = JSON.isJSON(cart)
		? Array.isArray(cart.entries)
			? cart.entries.length
			: 0
		: 0;
				...
				<List>
					<ListItem button>
						<ListItemText
							primary={
								<Link
									to={"/catalog".toUriWithLandingPagePrefix()}
									color="inherit"
								>
									Catalog
								</Link>
					...
					<ListItem button>
						<ListItemText
							primary={
								<Link
									to={"/recruitment".toUriWithLandingPagePrefix()}
									color="inherit"
								>
									Recruitment
								</Link>
			...
			<Hidden smDown>
				<HideOnScroll window={window}>
					<AppBar
						className={
							trigger
								? "shadow-xl inverse default_text"
								: "bg-transparent shadow-none default_text"
						}
						style={{
...
export default connect(mapStateToProps, {})(withErrorHandler(Header));
