...
class NotificationList extends Component {
	constructor(props) {
		...
		for (let [name, defination] of Object.entries(definations)) {
			this.contexts[UtilitiesHelper.singularize(name).toLowerCase()] = {
				icon: defination.icon ? defination.icon : <ContextlessNotificationIcon />,
	...
	render() {
		const { className, notifications, onSelect } = this.props
						...
						<div className={""}>
							<List component="div">
								{notifications.map(notification => (
									<Link key={notification._id} to={definations.notifications.access.actions.view.uri(notification._id)}>
										<ListItem className={""} component="div" onClick={onSelect}>
											<ListItemIcon
												className={""}
												style={{
													color: notification.context
														? notification.context.toLowerCase() in this.contexts
															? this.contexts[notification.context.toLowerCase()].color
											...
											>
												{notification.context
													? notification.context.toLowerCase() in this.contexts
														? this.contexts[notification.context.toLowerCase()].icon
												...
												primary={
													<React.Fragment>
														<Status
															variant="dot"
															color={
																notification.priority.toLowerCase() in priority_colors
																	? priority_colors[notification.priority.toLowerCase()]
																	: "#9e9e9e"
															}
															text={notification.title}
															text_color={notification.read ? colors.hex.grey : colors.hex.default}
														></Status>
													</React.Fragment>
												}
												secondary={
							...
							<div className={""}>
								<Button component={Link} to="/notifications" variant="text" color="primary">
									All Notifications
								</Button>
							</div>
						</div>
