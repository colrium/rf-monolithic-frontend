...
export default class Auth{
		...
		if (!!Auth.instance) {
			...
			if (authTokenLocation === "cookie") {
				...
				if (access_token && token_type) {
					this.access_token = { access_token: access_token, token_type: token_type, refresh_token: refresh_token };
				}
			}			
	...
	authorization_header(){
		let token = this.access_token;
		if (JSON.isJSON(token)) {
			return {Authorization: ("token_type" in token? (token.token_type+" ") : "")+("access_token" in token? token.access_token : "")};
		}	
		return null;		
	}
	...
	setAuthorizationHeader() {	
		if (JSON.isJSON(this.access_token)) {
			...
			if ("token_type" in this.access_token && "access_token" in this.access_token) {
				axios.defaults.headers.common['Authorization'] = `${this.access_token.token_type} ${this.access_token.access_token}`;
			}
