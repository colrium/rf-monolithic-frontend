...
const PageBuilder = asyncComponent(() => import("views/pages/Dashboard/PageBuilder"));
...
export default class DashboardRoutes extends Component {
	render() {
		...
		return (
			<Switch>
				<Redirect exact from={"/".toUriWithDashboardPrefix()} to={indexUri} />
				<Route path={"/home".toUriWithDashboardPrefix()} component={Dashboard}  authRestrict />
				<Route exact path={"/calendar".toUriWithDashboardPrefix()} component={RecordsListing} componentProps={{ context: "events" }}  authRestrict />

				{contexts.map((context, cursor) => (
					<Route exact path={context.toUriWithDashboardPrefix()} component={RecordsListing} componentProps={{ context: context }} key={"context-route-" + context}  authRestrict />
				))}

				{contexts.map((context, cursor) => (
					<Route exact path={(context + "/view/:id").toUriWithDashboardPrefix()} component={RecordView} componentProps={{ context: context }} key={"context-view-route-" + context}  authRestrict />
				))}

				{contexts.map((context, cursor) => (
					<Route exact path={(context + "/add").toUriWithDashboardPrefix()} component={RecordForm} componentProps={{ context: context }} key={"context-add-route-" + context}  authRestrict />
				))}

				{contexts.map((context, cursor) => (
					<Route exact path={(context + "/edit/:id").toUriWithDashboardPrefix()} component={RecordForm} componentProps={{ context: context }} key={"context-edit-route-" + context}  authRestrict />
				))}
				
				<Route path={"/page-builder".toUriWithDashboardPrefix()} component={PageBuilder}  authRestrict />
