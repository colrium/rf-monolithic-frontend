		...
		>
			{fileObjects.map((fileObject, i) => {
				if (
					...
					(UtilitiesHelper.isOfType(fileObject.attachment, "object") ||
						UtilitiesHelper.isOfType(fileObject.file, "object"))
				) {
					return (
						<ListItem className="p-0" key={"file-preview-list-item-" + i}>
							<ListItemAvatar className={classes.listItemAvatar}>
								
								{FilesHelper.isImage(fileObject.attachment? fileObject.attachment.name : fileObject.file.name ) ? (
										<Avatar
							...
							<ListItemText
								...
								primary={
									<Typography className="truncate" component="span" variant="body1" color="grey" >
									{ fileObject.progress ? ( fileObject.attachment ? fileObject.attachment.name : fileObject.file.name ) : (fileObject.attachment ?  fileObject.attachment.name  :  fileObject.file.name) }
									</Typography>
								}
								secondary={UtilitiesHelper.readableFileSize(
									fileObject.attachment ? fileObject.attachment.size : fileObject.file.size
								)}
							/>
							...
							<ListItemSecondaryAction className={classes.listItemSecondaryAction}>
								{fileObject.progress ? (
									<IconButton
										...
										edge="end"
										onClick={
											UtilitiesHelper.isOfType(handleCancelUpload, "function")
