					...
					config.options.map(
						(option, index) =>
							option in config && (
								...
								>
									{" "}
									{String.isString(config[option].icon) ? (
										<img src={config[option].icon} />
									) : (
										config[option].icon
									)}{" "}
								</IconButton>
...
class IconAndPopoverComponent extends Component {
	...
	constructor(props) {
		super(props);
		const { config } = props;
		this.state.id = String.isString(config.title)
			? config.title.variablelize()
			: String.uid(8);
...
class BlockTypeComponent extends Component {
	...
	render() {
		...
		const blockType = currentState.blockType
			? currentState.blockType.toLowerCase()
			: "normal";
		return (
			<div className="inline-block">
				<Button
					...
					{Array.isArray(config.options) &&
						config.options.map((option, index) => (
							<MenuItem key={"option-" + index}>
								<Typography
									variant={
										blockTypeVariants[option.toLowerCase()]
									}
								>
...
class FontFamilyComponent extends Component {
	...
	render() {
		...
		const fontFamily = currentState.fontFamily
			? currentState.fontFamily.toLowerCase()
			: "arial";
		return (
			<div className="inline-block">
				<Button
...
class WysiwygEditor extends React.Component {
	constructor(props) {
		...
		//check if HTML
		const isHTMLvalue =
			String.isString(converted_value) &&
			/<([A-Za-z][A-Za-z0-9]*)\b[^>]*>(.*?)<\/\1>/.test(converted_value);
	...
	render() {
		...
		let toolbarClassName =
			variant === "outlined"
				? "inline-block border-gray-400 border-b"
				: "";
		let wrapperClassName =
			variant === "outlined"
				? "border-gray-400 border rounded"
				: "rounded-t py-4";
		let containerClassName =
			(variant === "outlined"
				? " border-2 border-gray-400 rounded p-0"
				: "rounded-t py-4") + className
				? " " + className
				: "";
		...
		return (
			<GridContainer className={containerClassName}>
				{label && (
					<GridItem xs={12} className="p-0 px-2">
						{" "}
