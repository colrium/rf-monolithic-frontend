...
export default function LogoAnimation(props){
    const size = props.size ? props.size : 243.333;
    const circle_radius = props.thickness ? props.thickness : size/44;
    const left_path_x_points = [size / 2.1, size / 10.84, size / 2.1, size / 2.1, size / 2.1,];
    const left_path_y_points = [size / 1.35, size / 1.15, size / 7.47, size / 2.01, size / 1.35];
    const right_path_x_points = [size / 1.9, size / 1.1, size / 1.9, size / 1.9, size / 1.9];
    const right_path_y_points = [size / 7.47, size / 1.15, size / 1.35, size / 2.01, size / 7.47];
    ...
    return (
        <svg version="1.1" viewBox={"0 0 " + size + " " + size} width={size} height={size}>
            <motion.circle
                ...
                animate={{ cx: [null, ...left_path_x_points], cy: [null, ...left_path_y_points] }}
                transition={{
                    loop: Infinity,
                    ease: "linear",
                    duration: 1,
            ...
            <motion.circle
                ...
                animate={{ cx: [null, ...right_path_x_points], cy: [null, ...right_path_y_points] }}
                transition={{
                    loop: Infinity,
