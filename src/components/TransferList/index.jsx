...
class TransferList extends React.Component {
  ...
  handleToggle = option => event => {
    if (this.state.value.indexOf(option) !== -1) {
      if (this.state.checked_value_options.includes(option)) {
        ...
        checked_value_options.remove(checked_value_options.indexOf(option));
        this.setState(state => ({
          checked_value_options: checked_value_options
        }));
      } else {
        this.setState(state => ({
          checked_value_options: state.checked_value_options.concat([option])
        }));
      }
    } else {
  ...
  optionsList(values, list_type = "non-value-options") {
    ...
    return (
      <Paper className={classNames(classes.paper, classes.options_wrapper)}>
            ...
            return (
              ...
              >
                <ListItemIcon>
                  <Checkbox
                    checked={
                      (list_type === "non-value-options" &&
                        this.state.checked_options.includes(value_entry)) ||
                      (list_type === "value-options" &&
                        this.state.checked_value_options.includes(value_entry))
                    }
                    tabIndex={-1}
        ...
        <Grid item className={classes.actions_wrapper}>
          <Grid container direction="column" alignItems="center">
            <Button
              ...
              disabled={
                Object.keys(value_options).length === this.state.value.length
              }
            ...
            <Button
              ...
              disabled={this.state.checked_value_options.length === 0}
