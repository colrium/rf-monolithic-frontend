...
const tooClose = (activity, _activity) => {
  return (
    activity.left + activity.width === _activity.left ||
...
const overlap = (activity, _activity) => {
  return (
    (activity.left <= _activity.left &&
      activity.left + activity.width >= _activity.left + _activity.width) ||
    (activity.left >= _activity.left &&
      activity.left <= _activity.left + _activity.width)
  );
};
...
export default class Timeline extends React.Component {
  ...
  calculateData(props) {
    ...
    const years = Array.from(
      new Set(
        activities.reduce((previous, activity) => {
          const start = activity.start
            ? parseInt(moment(activity.start).format("Y"), 10)
            : currentYear;
          const end = activity.end
            ? parseInt(moment(activity.end).format("Y"), 10)
            : currentYear;
      ...
      .reduce((previous, year) => {
        ...
        return [...previous, ...gap.reverse(), year];
      }, [])
    ...
    const positionedActivities = activities
      ...
      .reduce((previous, activity) => {
        ...
        const lineIndexes = previous
          .filter(
            _activity =>
              overlap(activity, _activity) || tooClose(activity, _activity)
          )
          .map(_activity => _activity.row)
          .sort();

        activity.row = !lineIndexes.length
          ? 0
          : lineIndexes.reduce((_previous, row) => {
              if (row - _previous === 1) {
                return row;
              }
    ...
    const [maxWidth, minHeight] = positionedActivities.reduce(
      (previous, value) => {
  ...
  onTouchMove = e => {
    ...
    this.setState({
      positionX: Math.min(0, this.state.baseX + difference) + "px"
    });
  };
  ...
  onMouseMove = e => {
    ...
    const max =
      typeof e.currentTarget.querySelectorAll === "function"
        ? Array.from(e.currentTarget.querySelectorAll(".activity")).reduce(
            (previous, element) => {
  ...
  stateToNext = state => {
    ...
    return {
      slideIndex: Math.min(
        this.positionedActivities.length - 1,
        state.slideIndex + 1
      ),
      positionX
    };
  ...
  render() {
    const { minHeight, maxWidth, positionedActivities, years } = this;
      ...
      <div className="timeline">
        <div className="gallery">
          <div
            ...
            className={`previous${
              this.state.slideIndex <= 0 ? " disabled" : ""
            }`}
          />
          <div
            onClick={this.toNext}
            className={`next${
              this.state.slideIndex >= positionedActivities.length - 1
                ? " disabled"
                : ""
            }`}
          />
          ...
          >
            {positionedActivities.map(
              ({ body, image, name, moreLink }, index) => {
                return (
                  <div
                    className={
                      "activity" +
                      (this.state.slideIndex === index ? " focus" : "")
                    }
                    key={index}
                  >
                    <div className="container">
                      ...
                      <div className="card">
                        ...
                        <p>{body}</p>
                        {moreLink && (
                          <a className="external-link" href={moreLink.url}>
                            {moreLink.text}
                          </a>
                        )}
                      </div>
