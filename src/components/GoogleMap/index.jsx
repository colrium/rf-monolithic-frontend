...
const GoogleMap = React.forwardRef((props, ref) => {
		...
		return {
			lat:
				Array.isArray(center) && center?.length === 2
					? center[0]
					: center?.lat || center?.latitude || geoLocation?.latitude || google_maps?.default_center?.lat,
			lng:
				Array.isArray(center) && center?.length === 2
					? center[1]
					: center?.lng || center?.longitude || geoLocation?.longitude || google_maps?.default_center?.lng,
		}
	}, [center])


	const showInfoWindow = useCallback((content, position, options = {}) => {
	...
	const getPolylineBounds = useCallback(polyline => {
		let bounds = null
	...
	const applyEntrySelection = useCallback((type, index) => {
		if (!!googlemapRef.current) {
			...
			if (type === "polyline" && !!polylinesRef.current[index]) {
				...
				if (!!google) {
					new google.maps.event.trigger(circlesRef.current[index], "click")
				}
			}
			if (elementBounds) {
	...
	const applyCircles = useCallback(() => {
		...
		if (Array.isArray(circles) && !!google) {
			let entriesZindexes = circles
				.sort((a, b) => b.radius - a.radius)
				.reduce((currentZindexes, current, index) => {
			...
			circlesRef.current = circles.map((newCircle, index) => {
				...
				let mapElement = new google.maps.Circle({
					...
					options: {
						...
						strokeColor: elementProps.color || colors.hex.default,
				...
				google.maps.event.addListener(mapElement, "click", function (event) {
					if (!!googlemapRef.current) {
						...
						googlemapRef.current.fitBounds(mapElement.getBounds())
					}
				})
				google.maps.event.addListener(mapElement, "mouseover", function (event) {
					mapElement.setOptions({ fillOpacity: 0.5 })
					infoWindowTimeoutRef.current = setTimeout(
						() =>
							showInfoWindow(
								infoWindow ? ReactDOMServer.renderToStaticMarkup(infoWindow) : elementProps.title ? elementProps.title : "",
								mapElement.center
							),
						1000
					)
				})
				google.maps.event.addListener(mapElement, "mouseout", function (event) {
			...
			if (circlesRef.current.length > 0 && !!circlesRef.current[0].getBounds && !!googlemapRef.current) {
				googlemapRef.current.fitBounds(circlesRef.current[0].getBounds())
			}
		}
	}, [circles])
		...
		if (Array.isArray(polylines) && !!google) {
			polylinesRef.current = polylines.map((element, index) => {
				...
				let mapElement = new google.maps.Polyline({
					options: {
						fillColor: elementProps.fillColor || elementProps.color || colors.hex.default,
						strokeColor: elementProps.strokeColor || elementProps.color || colors.hex.default,
						strokeOpacity: 1,
						fillOpacity: 1,
						strokeWeight: 1.5,
						zIndex: 1,
						...
						(infoWindow ? ReactDOMServer.renderToStaticMarkup(infoWindow) : elementProps.title ? elementProps.title : "") +
							(event?.latLng
								? "<br/> Crow fleight distance from Start: " +
								  ...
								  " Km"
								: ""),
						event?.latLng || mapElement?.latLngs[mapElement?.latLngs?.length - 1]
					)
				})
	...
	const applyGeojsons = useCallback(() => {}, [geojsons, selectedEntryType, selectedEntry])
