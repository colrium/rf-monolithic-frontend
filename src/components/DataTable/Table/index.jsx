...
export default function TableComponent(props) {
	...
	const sorting = useMemo(() => {
		...
		if (!String.isEmpty(sort)) {
			sortingValue = sort.split(",").reduce((currentOrdersValue, entry) => {
				currentOrdersValue[entry.replace("-", "")] = entry.startsWith("-") ? "desc" : "asc"
	...
	const handleOnClickRowCell = React.useCallback(
		(data, header) => e => {
			if (JSON.isJSON(data) && Function.isFunction(header?.resolveReferenceText)) {
				setState({
					objectDialog: {
						open: true,
						src: data,
	...
	const isSelectible = useCallback(
		row => {
			return selectibleRows === null || (selectibleRows !== null && selectibleRows.includes(row[identifier]))
		},
		[selectibleRows]
	)
	...
	const formatContent = useCallback(
		(header, content, additionalContent = null) => {
			...
			if (header.datetime) {
				return `${format(parseISO(content), "d MMM yyyy")} ${format(parseISO(content), "h:mm:ss a")}`
			}
			if (header.time) {
	...
	const handleContextMenu = useCallback(event => {
		...
		setState(prevState => ({
			contextMenu: prevState.contextMenu === null ? { mouseX: event.clientX - 2, mouseY: event.clientY - 4 } : null,
		}))
	}, [])
			...
			return headers
				.map(header => {
					if (header.arrayAttribute) {
						return (JSON.get(row, header.childAttribute) || []).length
					}
					return 1
				})
			...
			<Paper sx={{ position: "relative", width: "100%", mb: 2 }}>
				{showToolbar ? (
					<TableToolbar
						...
						handleActionClick={(event, action) => handleActionClick(event, action)}
				...
				<ScrollBars>
					<Table aria-labelledby="tableTitle" size={dense ? "small" : "medium"} aria-label={title} stickyHeader>
						<DataTableHead
							headers={headers}
							order={order}
							orderBy={orderBy}
						...
						/>
						<TableBody>
							{loading && Object.keys(headers).map((key, row) => (
								<TableRow key={`placeholder-row-${row}`}>
									{!!headers && Object.keys(headers).map((key, cell) => (
										<TableCell className={"p-4 text-center"} key={`placeholder-row-${row}-cell-${cell}`}>
											{cell === 0 && <Box className="flex flex-row place-content-evenly">
												<Skeleton variant="circular" className="mx-1" width={32} height={32} />
												<Skeleton variant="circular" className="mx-1" width={32} height={32} />
												<Skeleton variant="circular" className="mx-1" width={32} height={32} />
											</Box>}
											{cell > 0 && <Skeleton variant="text" />}
										</TableCell>
									))}
								</TableRow>
							))}
							{!loading && Array.isArray(rows) && rows.length === 0 && (
								<TableRow>
									<TableCell className={"p-4 text-center"} colSpan={headers?.length || 1}>
										<Stack
											className="p-0 m-0"
											justify="center"
											alignItems="center"
										...
										>
											<FolderOpenIcon className={"m-4 capitalize"} sx={{ fontSize: 56 }} color="inherit" />
											<Typography className={"m-4 capitalize"} variant="subtitle2" fullWidth>
												No data
											</Typography>
										</Stack>
									</TableCell>
															...
															<Box margin={1}>
																{collapseContent ? (
																	...
																	<Table size="small" aria-label="details">
																		...
																		<TableBody>
																			<TableRow>
																				{collapseHeaders.map(head => getCellDom(head, row))}
																			</TableRow>
																		</TableBody>
																	</Table>
																)}
