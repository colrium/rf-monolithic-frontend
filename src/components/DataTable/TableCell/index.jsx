...
export const DataTableCell = React.forwardRef((props, ref) => {
	const { data, header, selected, key, htmlContent,  collapseOpen, onClickCollapseIcon, truncate, checkboxDisabled, disabled, onClick, disableCopyToClipboard, ...rest } = props;
	...
	const formatContent = useCallback((header, content, additionalContent = null) => {
		if (!content) {
			return "";
		}
	...
	if (header.collapse) {
			...
			return (
				<TableCell
					className={`text-sm text-center ${headerClassName? headerClassName : ""}`}
					key={key}
					padding="checkbox"
					ref={ref}
				>
										...
										<IconButton
											...
											style={{
												color:
													...
													disableCondition(
														data
													)) || disabled ? grey[400] : color || "currentColor",
											}}
											disabled={
												(disableCondition &&
													disableCondition(
