...
class AttachmentsApiService extends ApiService{
	...
	async upload(data, params={}) {
		...
		let that = this;
		return await this.apiInstance.post(endpoint_uri, data, params)
	...
	async download(attachment) {
		...
		if (attachment_rec === null) {
			attachment_rec = await this.getRecordById(id, {}).then(res => {
	...
	getAttachmentFileUrl(attachment){		
		return this.base_url+this.service_uri+'/download/'+(JSON.isJSON(attachment) && "_id" in attachment? attachment._id : attachment);
	}

	async getAttachmentFile(attachment) {
		let endpoint_uri = this.service_uri + '/download/' + JSON.isJSON(attachment) ? attachment._id : attachment;

		let response = await this.apiInstance.get(endpoint_uri, {}, {responseType: 'stream'}).then(({ data }) => {
	...
	async getAttachmentFileById(id) {
		...
		return this.apiInstance.get(this.service_uri+'/'+id, {}).then((res)=>{
			let attachment = res.data.data;
			return this.apiInstance.get(file_url, {}, {responseType: 'stream'}).then(({data})=>{
				return data;
			});
		})
...
export default new AttachmentsApiService();