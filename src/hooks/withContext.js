	...
	function WithContext(props) {
		...
		const fetchSettings = ()=>{
			...
			SettingsService.getRecords({}).then(res => {
				...
				defaultContext = {...context, settings: fetchedSettings, settingsFetched: true};
		...
		const updateContextSettings = async (name, value)=>{
			if (String.isString(name)) {
				...
				let updatedContext = await SettingsService.update(slug, postData, {create: 1}).then(res => {
					return {...context, settings: { ...context.settings, [slug]: value }}
				}).catch(e=>{
					return false;
				});
				if (updatedContext) {
		...
		if (isFunctionalComponent(Component)) {
			return (
				<AppContext.Provider value={context}>
					<Component {...state} context={context} updateContextSettings={updateContextSettings} />					
				</AppContext.Provider>
			);
		}
		else{
